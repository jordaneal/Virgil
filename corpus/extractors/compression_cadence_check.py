...
# D5: Condition-recovery polysemy (Patch 1).
# "come to consciousness" variants that fire OVERNIGHT_REST but actually
# describe a PC waking from being knocked out (combat) or abducted (transport).
# Two-part guard: phrase must be a consciousness variant AND contextual signals
# must confirm combat/abduction context in the turn or preceding 3 turns.

CONSCIOUSNESS_VARIANTS = re.compile(
    r"\bcom(?:e|es|ing)\s+to\s+(?:consciousness|awareness|your\s+senses)\b",
    re.I,
)

CONDITION_RECOVERY_SIGNALS = re.compile(
    # HP-restoration / combat-state vocabulary
    r"\bhit\s+points?\b"
    r"|\bHP\b"
    r"|\b\d+\s+(?:hit\s+)?points?\s+of\s+damage\b"
    r"|\bknocked\s+(?:out|unconscious)\b"
    r"|\bdropped\s+to\s+(?:zero|0|nothing)\b"
    r"|\btentacles?\b"
    # Active-combat sound effects
    r"|\(gunshot\)|\(impact\)|\(spray\)|\(hoofbeats\)"
    # Abduction / transport-while-captive markers
    r"|\bhooves?\b"
    r"|\bon\s+the\s+back\s+of\s+(?:a\s+|this\s+|the\s+)?horse\b"
    r"|\btied\s+tightly\b"
    r"|\bkidnapped\b"
    # Combat-initiative turn announcement
    r"|\bit'?s\s+your\s+turn\b",
    re.I,
)


def is_condition_recovery(turn_text, preceding_text):
    """True if a 'come to consciousness' OVERNIGHT_REST candidate is really condition recovery.

    Requires both: (a) consciousness-variant phrase in the turn, and
    (b) a combat/abduction/HP signal anywhere in turn + preceding 3 turns.
    """
    if not CONSCIOUSNESS_VARIANTS.search(turn_text):
        return False
    return bool(CONDITION_RECOVERY_SIGNALS.search(turn_text + " " + preceding_text))
...
def stage_0_phrase(turn_text, phrase_start, phrase_end, family, preceding_text=""):
    ...
    preceding_text: concatenated text from up to 3 preceding MATT turns, used
    for D5 context-aware condition-recovery check.
    """
    ...
    # D5: Condition-recovery polysemy (OVERNIGHT_REST only).
    # "come to consciousness" variants that describe a PC waking from being
    # knocked out mid-combat or from abduction/transport — not overnight rest.
...
def process_episode(episode_id, extracted_at):
    ...
    for idx, t in enumerate(turns):
        ...
        # Preceding text for D5 condition-recovery check: last 3 turns (all speakers).
        pre_start = max(0, idx - 3)
        preceding_text_d5 = " ".join(turns[j]["text"] for j in range(pre_start, idx))
