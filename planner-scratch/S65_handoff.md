...
# S65.A — Format Unification (Same-Session Followup)

**Trigger.** Post-restart Discord verify of Fix 3 surfaced format inconsistency: the boxed-bookkeeping shape (bullet + backticked command) looked cleaner than the bold-wrapped roll directive. Jordan requested unification across all mechanical-instruction surfaces.

**Two iterations:**

### Iteration 1 — bullet + backticked box, drop preambles
- Match all command renders (rolls, attacks, mechanical hints, Suggested Actions) to the bullet + backticked-box format.
- Remove "Bookkeeping (you type these):" preamble + horizontal divider in `_attach_hints`.
- Remove "Suggested Actions:" header preamble in `build_dm_context`.

### Iteration 2 — actor name OUTSIDE the box
- Box must contain ONLY bare Avrae syntax (no actor name, no separator).
- Shape: `- **<Actor>:** \`<bare command>\`` — bullet + bold-actor-prefix + colon, then backticked bare command.
- Rationale: player can copy-paste the exact box contents to Avrae without editing.

**Files touched (S65.A):**

| File | Change |
|---|---|
| `dnd_orchestration.py` | `to_prompt_directive` skill/save branches: render `- **<First Name>:** \`{cmd} {dc_int}\``. Attack branch: render `- **<First Name>:** \`!attack <weapon-name> -t <target>\`` + `- **<First Name>:** \`!cast <spell-name> -t <target>\``. Removed legacy `**!check skill DC : Actor**` bold-around-command wrap. |
| `discord_dnd_bot.py` | `_LLM_EMIT_DIRECTIVE_RX` and `_DC_STRIP_RX` updated to accept BOTH `**` (legacy) and `` ` `` (current) boundary chars — graceful coexistence in regex parsers so any stale narration echo doesn't break parsing. `_attach_hints` (lines 2742-2750) drops preamble + divider; outputs only `- \`{hint}\`` bullets. |
| `dnd_engine.py` | `build_dm_context` Suggested Actions block: drop "Suggested Actions:" header preamble, switch format examples to bullet + backtick. Downstream parsers (lines 7626-7647) re-anchor on bullet pattern (`- ` or `- /`) instead of literal "Suggested Actions:" header. |

**Tests (S65.A):**

| Test file | Status | Notes |
|---|---|---|
| `test_format_unification_closure.py` | 23 new tests, all pass | Verifies actor-outside-box shape, graceful legacy coexistence in regex, `_attach_hints` no-preamble, Suggested Actions backtick format. Test names: `test_skill_roll_uses_actor_outside_box`, `test_save_roll_uses_actor_outside_box`, `test_skill_roll_box_does_not_contain_actor`, `test_attack_template_uses_actor_outside_box`, `test_strip_preserves_backtick_wrap_actor_outside`, `test_parser_recognizes_backtick_bounded_actor_outside`. |
| `test_attack_directive.py` | 50/50 pass | Assertions updated to actor-outside-box shape (`- **Donovan:** \`!check stealth 15\``). Removed legacy bold-around-command asserts. |
| `test_dc_less_roll_closure.py` | 25/25 pass | Updated `test_directive_renders_dc_as_literal_integer` to assert new shape. |
| `test_advisory.py` | 31/31 pass | Unchanged in S65.A. |
| `test_dm_aside_role_closure.py` | 13/13 pass | Unchanged in S65.A. |
| `test_play_smoke.py` | 6/6 pass | Unchanged in S65.A. |

**Live verification.** Playtest at 9:47 AM 2026-05-14 confirmed render:
```
Ruby: !check persuasion
```
(`Ruby:` bold outside, `!check persuasion` in a gray inline-code box.) Format applies across rolls, attacks, bookkeeping hints, and Suggested Actions.

**Rollback procedure.** Revert `to_prompt_directive` skill/save/attack branches in `dnd_orchestration.py` to the pre-S65.A bold-around-command template. Revert `_attach_hints` and the two regex updates in `discord_dnd_bot.py`. Revert `build_dm_context` Suggested Actions block + downstream parsers in `dnd_engine.py`. Backtick-tolerant regexes can stay (defense-in-depth for any other emit surface).

---

# Playtest Findings — Baker Pricing Scenario (2026-05-14T09:47–09:49)

**Context.** Post-S65.A playtest by Jordan as Ruby Christine in Larethil's Market Square. Five-turn baker pricing scenario surfaced four NEW issues, all **outside S65 scope** but caught by adversarial play.

### Issue 1 — NPC retcons free-loaf transaction into "half-price deal"
- 9:47: persuasion check (1d20+5 = 20) succeeds. Baker gives ONE loaf **free** ("Consider it on the house, dear").
- 9:49: dispute turn. Baker re-narrates: "your **half-price** was a kindness for a traveler's song, not a new rate."
- **Root cause.** Engine has no NPC transaction-state memory. Each turn the LLM invents a fresh pricing story consistent with the current narration target, regardless of prior commitments. Free → "half-price" is a 1-turn drift.
- **Scope.** NPC commitment-tracking layer. Multi-spec; not actionable as a single fix.

### Issue 2 — Bookkeeping hint fires on dialogue, not on actual transactions
The `!game coin` hints fire whenever the LLM's narration *mentions* a price:
...
- **Root cause.** Hint extractor parses LLM narration; LLM re-states prices on every dispute turn; each restatement becomes a fresh `!game coin` proposal. Actual purchase has no hint because the LLM didn't explicitly state the price in that narration.
- **Net effect if player follows hints verbatim:** loses `6sp` for a `5sp` (or `2.5sp`) transaction.
- **Scope.** Hint extractor logic change. Single-surface fix candidate — extract only when narration includes an explicit transaction-completion verb (paid/handed over/accepted payment) AND a price, not on bare price mentions.

### Issue 3 — NPC confirms hallucinated player history
- Jordan: "they were 50c each before."
- Baker: "I'm still charging a silver a loaf" — implicitly confirms a prior price existed (none did).
- **Root cause.** No anti-gaslight rail forcing NPCs to flag "I don't recall stating that" when the player references prices/facts not in scene state.
- **Scope.** NPC scene-state honesty rail. Multi-spec; entangled with Issue 1.

### Issue 4 — Inconsistent totals across dispute turns
- Established free (9:47).
- Quoted 5sp/5 loaves (1sp each) (9:48).
- Player asserts 50c each (9:48).
- Baker re-quotes 1sp/loaf (9:49a).
- Final answer: 5sp/5 loaves (9:49b).
- **Net.** Engine can't internally reconcile what was said one turn ago. Each fresh narration is unanchored.
- **Scope.** Same as Issue 1.

**Triage recommendation.**
- **Issue 2** (hint firing on dialogue) is the single actionable extractor-side fix in S65.1+ scope. Estimated 1-day spec + 1-day implement + adversarial verify. No NPC-memory architecture needed.
- **Issues 1 + 3 + 4** imply a unified **NPC commitment-tracking layer** — multi-spec scope. Candidate filing for the longer-horizon backlog (alongside F-077 filled-clock and the NPC-memory threads in `PROJECT_LONG_HORIZON_REVIEW.md`).

See `S65_1_candidates.md` for the formal candidate filing.
