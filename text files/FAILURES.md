...
- [§F-58. Stale-footer name parsing — DM directs check by name, footer disagrees](#f-58-stale-footer-name-parsing)
- [§F-59. Avrae bot-filter — bot-emitted `!`-commands silently filtered at parser](#f-59-avrae-bot-filter)
- [§F-60. Three-pass scope-incomplete inventory — symptom-patching when named-block list is wrong](#f-60-three-pass-scope-incomplete)
- [§F-61. Instruction-side force-suppress insufficient when routing fires through different path](#f-61-instruction-side-suppress-insufficient)
- [§F-62. Sync-direction race — `push-docs` clobbered server-side Code edits](#f-62-sync-direction-race)
- [§F-63. Filed follow-up scoped at wrong table (planner-side discipline gap)](#f-63-filed-follow-up-wrong-table)
- [§F-64. (CANDIDATE) LLM-extracted activity signals as perverse-incentive surfaces in stagnation-detection contexts](#f-64-llm-extracted-activity-signal-perverse-incentive)
...
## §F-58. Stale-footer name parsing

**Surfaced:** Session 32 — Bug 1 Phase 1 spec drafting.
**Context:** Bug 1 Phase 1 (S32) ships a roll-directive matcher that snapshots `dnd_scene_state.last_active_actor` at directive-emit time. The DM types `!check stealth` and the matcher binds the directive to whoever the footer says is the active actor. But the DM may type `!check stealth` *meaning* a different PC than the footer shows — e.g. the footer was last updated when Donovan responded, but the DM has just narrated a beat directed at Hilda (no `_dm_respond_and_post` call because Hilda hasn't actually responded yet) and types `!check stealth` intending Hilda. Phase 1 strict-binds to Donovan (the footer's `last_active_actor`); Hilda's roll arrives and trips `directive_actor_mismatch:` + the wrong-actor aside.
**Failure mode:** Strict footer-binding is correct under the locked architecture but misses the natural-language case where the DM addresses a player in narration without that player having responded yet (footer hasn't transitioned). Symptom: aside posted unnecessarily; directive never resolves; row sits until TTL expires.
**Disposition:** **Filed as v1.1 candidate.** Phase 1 deliberately strict-binds and observes the rate via `directive_actor_mismatch:` log volume. v1.1 candidate fix shape: parse the DM's narration text (or directive line) for an explicit name reference (e.g. `Hilda, !check stealth` or "Hilda picks the lock — `!check stealth`") and prefer the parsed name over the footer when present. Doctrine §14 (strict literal match + telemetry > fuzzy) endorses the wait-for-data approach. Cross-references: `BUG_1_SPEC.md` §F.2 / §N (v1.x candidates filed). Anchor as a real fix only if Phase 1 logs show the pattern is routine rather than rare. Phase 2 spec consideration: parse-explicit-name vs strict-footer-binding is a conscious design call, not a default.
...
## §F-59. Avrae bot-filter — bot-emitted `!`-commands silently filtered at parser

**Surfaced:** Session 41 — Ship 3 NPC State-Sync Boundary live verify (HALT-and-pivot first arc instance).
**Context:** Ship 3 spec locked candidate (a): bot emits `!init opt <name> -hp <hp> -ac <ac>` directly to `#dm-narration` under proposed Doctrine §65a narrow exception (bot-as-DM-proxy for hydrate/Avrae-sync flow). Live verify ran the locked architecture cleanly: bot composed the correct command, posted to the right channel, no exception thrown, log line confirmed emission. **Avrae did not respond.** Donovan Ruby's HP remained at the pre-emit value. Operator then human-typed the *identical command string* into the same channel; Avrae responded instantly and applied the change.
**Failure mode:** Avrae's parser structurally filters bot-emitted `!`-commands. Identical command strings mutate state when human-typed and are silently filtered (no error, no rejection embed, no log entry on Avrae's side) when bot-typed. This is a third-party API boundary, not a Virgil bug — Avrae deliberately distinguishes bot vs human authors at command ingestion. The failure is invisible from Virgil's perspective: emission succeeds, no exception, no rejection signal, only the absence of expected state change reveals it.
**Architectural relationship:** Dissolves Doctrine §65a candidate entirely — the candidate's whole premise ("narrow exception for bot to emit `!init opt` for hydrate sync") was structurally impossible to fulfill regardless of doctrine. Reinforces the existing locked invariant: bot stays read-only on the Avrae channel, all `!`-commands route through the LLM in narration responses. Project-side proposals of mechanical state mutation route through the **§1b validated-suggester pattern**: bot proposes via `#dm-aside`, deterministic gate validates, DM approves by paste, Avrae executes (because the DM is human, so Avrae's parser accepts the command). Ship 3 pivoted in-session to candidate (a'): bot posts copy-paste 3-line sync block to `#dm-aside`; DM pastes each line as a separate message (Avrae filters batched commands as well — separate empirical finding from same verify). Closes Finding H via the suggester surface instead of direct emission. **First instance of HALT-and-pivot pattern** — spec lock at architectural shape + verify at implementation surfaces an empirical wall + operator decision license for in-session pivot.
**Disposition:** §1b second project instance proven (joins Track 6 #5.1 SRD suggester as the canonical answer). §65a candidate dissolved permanently. WORKING_WITH_CLAUDE.md anti-pattern catalog updated with the empirical reinforcement: "Avrae structurally filters bot-emitted `!`-commands." Future ships proposing bot→Avrae writes must find this entry and route through §1b instead. The structural protection: any new mechanical-command surface is added to the LLM's emission repertoire, never as new bot→Avrae write paths. **Doctrinal weight:** the empirical-finding-from-live-verify pattern is the load-bearing piece — the spec was correctly designed against the doctrine as it stood pre-S41; the doctrine itself had a structural gap that only live verify could surface. Spec lock + verify + in-session pivot is the cadence working as designed, not a failure of spec discipline.

## §F-60. Three-pass scope-incomplete inventory — symptom-patching when named-block list is wrong

**Surfaced:** Session 44 — Combat narration prompt purity v1.x (HALT-and-pivot second arc instance).
**Context:** S43 dumb combat shipped clean per §77 doctrine line (no adjudication drift in any verify). ROUND_START verify surfaced storytelling-quality drift — phantom NPCs from companion block, stale-narrative bleed. Filed v1.x. S44 was the follow-up ship to close the bleed via information-side suppression. **First-pass scope** identified two bleed sources from planner-side analysis: `recent_npcs_line` (Recently active NPCs render in SCENE STATE block) and chroma session retrieval (`=== RELEVANT PAST EVENTS ===`). P1 shipped 2-block suppression set. Verify: drift continued, but with different symptoms. **Second-pass scope** added 7 more blocks after Code did a full prompt audit of `build_dm_context`: companions, quests, inventory, central_thread, pending_consequences, unresolved_commitment, plus the chroma knowledge-corpus block (`=== DM PACING EXAMPLES ===` from the 740k corpus). P2 shipped 9-block scope. Verify: drift continued at low rate — a specific ROUND_START emission still mentioned a downed goblin verbatim from a prior combat. **Third-pass scope** required direct DB inspection of `dnd_campaigns.current_scene` (live SQLite query, not prompt audit). Surfaced the rolling-narration buffer: `current_scene` is overwritten with the LLM's last narration verbatim on every `_dm_respond_and_post`, then re-injected as `=== CURRENT SCENE ===` on the next prompt. P3 added it to the suppression set (10 blocks total) and verify ran clean.
**Failure mode:** Planner-side analysis named two bleed sources from doc-side reasoning. Actual bleed surface was ten sources. The first-pass fix-shape was right (information-side suppression of context blocks) but the named-block list was incomplete by 8 sources. Symptom-patching cycle: each pass closed one symptom and surfaced the next without forcing exhaustive inventory of the failure surface upfront. Three iterative ships consumed three live-verify cycles when one exhaustive inventory pass + one ship would have closed it.
**Architectural relationship:** Distinct from architectural-shape errors (which are spec-time failures) and distinct from drift-after-fix (which is the underlying issue this discipline addresses). This is a **scope discipline failure** — when a fix-shape patches but drift continues, the next move is exhaustive inventory of the failure surface, not another speculative patch. The discipline pattern earned doctrinal weight as "inventory before patch; evidence before speculation" in WORKING_WITH_CLAUDE.md S33-S45 arc patterns. Sibling discipline: **DB inspection beats prompt audit** when block-suppression is exhaustive but drift continues. The S44 P3 finding (rolling-narration buffer in `current_scene`) would not have come from a fourth prompt audit pass — it required direct inspection of which fields are live state. When two patches close named bleed sources and drift continues, the next move is "what writes are live that the inventory hasn't covered," not "tighten the prompt instructions further."
**Disposition:** Closed by S44 P3 ship (10-block suppression set verify clean). Discipline pattern anchored in WORKING_WITH_CLAUDE.md Workflow refinements section (S33-S45 arc). Two-layer enforcement composition observation filed (§77 instruction-side from S43 + information-side suppression from S44); awaits 3rd instance to anchor as separate §-entry. **Doctrinal weight:** the three-pass cycle was the cadence working as designed once HALT-or-extend escape hatch was used — each pass narrowed via evidence, not speculation. What the discipline catches is doing the inventory upfront instead of letting empirical narrowing eat three sessions of verify-restart-test. Future ships that hit "fix patches symptoms but drift continues" find this entry and run inventory first.

## §F-61. Instruction-side force-suppress insufficient when routing fires through different path

**Surfaced:** Session 45 — Combat-boundary hardening bundle, Surface D v1→v2 upgrade (HALT-and-pivot third arc instance).
**Context:** S45 Surface D shipped a defense-in-depth gate in `_dm_respond_and_post`: when `mode='combat'` AND no `active_turn`, force `suppress_for_combat_narration=True` to prevent premature combat narration during init-setup window. Verify pass 2 ran: operator typed `!init begin` (combat mode flipped), Avrae prompted for combatant disambiguation (`Multiple matches found: 1. Goblin 2. Hobgoblin`), operator typed bare `2` to disambiguate. **The bot generated premature combat narration from the bare `2` reply.** Suppression was in effect (telemetry confirmed `init_setup_suppression: fired=1`), but the narration was already mid-stream before the gate could apply.
**Failure mode:** v1 gate placement was inside `_dm_respond_and_post`, which is the narration-execution path. But the bare `2` reply triggered `on_message` first, which fired into the existing turn-gate/batcher flow at the top of the routing path, **before** `_dm_respond_and_post` was reached. By the time the suppression gate fired, the narration prompt had already been assembled and dispatched to the LLM. Suppression at the wrong layer — instruction-side enforcement (force-suppress params downstream) cannot protect against narration that fires through a different routing layer (top-level on_message routing upstream). The architectural mistake was placing the gate at the execution surface rather than the routing surface.
**Architectural relationship:** Sister failure to F-49 (verifier vs hydration paired — output-layer fabrication detection requires input-layer hydration support to close fully). Same shape: a single layer of protection at the wrong surface is structurally insufficient; two layers at the right surfaces close the failure mode. v2 fix added a **top-level gate in `on_message`** above the existing turn-gate (~line 2126+): when `mode='combat'` AND no `active_turn`, react ⏳ and return BEFORE the batcher accepts the message. Bot stays silent during init-setup; ROUND_START dispatch provides the proper atmospheric opener when Avrae announces the first turn. v1 gate retained as defense-in-depth for non-`on_message` call sites (slash commands, `_handle_dm_roll_arrival` auto-fire, `/travel`). Two-layer structural protection now matches the two-layer enforcement composition pattern §77/§78 anchored.
**Disposition:** Closed by S45 D v2 in-session upgrade. Pass 3 verify clean across all three surfaces (Surface C buffer reset + Surface D v2 + Surface F COMBAT_END auto-closeout). HALT-and-pivot third arc instance — operator's standing "do them all" directive enabled in-session iteration through v1→v2 upgrade rather than re-spec. **Doctrinal weight:** the failure mode names a structural rule that landed as Doctrine §78 layer 3: *transitional-window structural silence when mode flipped but mechanical state isn't fully populated*. Mode-flag-only transitions are structurally incomplete — the four-layer rule (mechanical cleanup + narrative buffer reset + transitional silence + boundary atmospheric closeout) requires all four layers, and the silence layer requires top-level routing protection, not execution-surface suppression. Future mode-transition handlers must place silence gates at the routing surface, not the execution surface.

## §F-62. Sync-direction race — `push-docs` clobbered server-side Code edits

**Surfaced:** Session 46.
**Context:** During S46, Code shipped a small hygiene patch (verifier_error sentinel constant + emit-site swap) that touched `narration_verifier.py`, `dnd_engine.py`, and `VIRGIL_MASTER.md` server-side at ~10:13. Operator ran `push-docs` (PC→server one-way rsync) at 10:40:29 without first running `backup-virgil` (server→PC). PC's `text files/VIRGIL_MASTER.md` was at yesterday-afternoon mtime — it had never received Code's 10:13 server edit. `push-docs` has no `--update` flag; content-differing files overwrite regardless of mtime. PC's stale copy clobbered server's fresh copy of `VIRGIL_MASTER.md`. The Python-side edits survived because they only flow server→PC (PC has no write authority on those paths).
**Failure mode:** Inverse of the prior-known S45 failure mode (server→PC `backup-virgil` clobbering PC-side planner edits). Same shape, opposite direction. WORKING_WITH_CLAUDE.md's Discipline section named only the S45 direction; the inverse direction was implicit-not-codified.
**Architectural relationship:** Two one-way pushes (`push-docs` PC→server, `backup-virgil` server→PC) act as a sync pair only when run in the correct order relative to edits. The structural protection is Code's existing Deployment workflow rule: targeted scp/rsync of files-edited-this-session as standard end-of-session cadence. The S46 incident occurred because Code skipped that targeted push on the sentinel ship, leaving PC stale. `push-all-to-pc.sh` is the wrong tool for this protection (Jordan-only per existing Deployment workflow rule) — the right tool is the targeted per-file rsync that was already discipline.
**Disposition:** Recovery: re-applied Code's sentinel bullet server-side, applied the VM:295 reconciliation in the same edit (unified bullet — both reconcile to one place); recovery prompt incorrectly invoked `push-all-to-pc.sh` (worked, but wrong tool). Doctrine reinforcement landed in WORKING_WITH_CLAUDE.md — inverse-failure-mode bullet added to the Discipline section, cross-references the existing Deployment workflow rule (Code uses targeted scp/rsync of files-edited-this-session; `push-all-to-pc.sh` is Jordan-only). Discipline framed as reinforcement-of-existing rather than new-patch.

## §F-63. Filed follow-up scoped at wrong table

**Surfaced:** Session 46 — first dispatch of S46 cleanup queue.
**Context:** S45 verify produced a small follow-up filing: "phantom companions DB hygiene in `dnd_companions` table." Planner accepted the filing as well-scoped without re-grepping the actual write path. When dispatched as the first cleanup ship, Code's recon (Phase 1) returned: `dnd_companions` is operator-written via `/companion add` — no LLM write path. The three rows in the table (Lira, Borin, Eldrin per skeleton.md) were authored canon, not phantom rows. Deleting them would have erased the band-of-three the campaign was designed around. The actual phantom problem the follow-up was trying to fix was in `dnd_npcs` (a different table) with LLM-extraction bare-firstname rows accumulating mention_count.
**Failure mode:** Planner-side discipline gap. Filed follow-ups from end-of-session handoff were treated as scoped specifications without recon-check. The follow-up's surface premise ("phantom companions") was right; the table identification was wrong; the difference was load-bearing because the two tables have different write paths and different fix shapes.
**Architectural relationship:** Distinct from filing-bloat patterns (§F-23 first spec proposed sequenced appendix) and from speculation-before-evidence (S44 inventory-before-patch). This is a specification-quality failure at the planner→Code handoff: the planner trusted a filed follow-up's premise without confirming the technical scope. Recon-first dispatch caught it, but the cost was a wasted session-start cycle and a near-wrong delete. The recon-first pattern's value is exactly this: Code's hands-on advantage catches specification errors the planner can't see from doc-side.
**Disposition:** Discipline pattern surfaced but not filed as candidate (single instance, S46). The pattern: when a filed follow-up names a specific table / file / write path / module, planner re-verifies the technical scope before treating the filing as a locked specification. Recon-first dispatch is the structural protection when the verification can't happen planner-side. If a second instance surfaces of a filed follow-up specifying the wrong technical surface, anchor as discipline candidate. For now: planner-side memo — filings are starting points, not specs.


## §F-64. (CANDIDATE) LLM-extracted activity signals as perverse-incentive surfaces in stagnation-detection contexts

**Status: CANDIDATE.** Not anchored. Awaits operator confirmation. Two project instances established; the pattern is named for future ships to inherit even if formal doctrine anchoring waits for a third instance.

**Surfaced:** S53 (NPC was_new live verify) + S63 (consequence upsert specced-but-never-wired retroactive framing). Both touched Scene Lifecycle v1's §1.F activity-signal set.

**Context — instance 1 (S53, §1.F.c NPC was_new):**
Scene Lifecycle v1 specced six §1.F activity signals to reset its stagnation counter. Two were derived from LLM narration via extractors: NPC was_new (npc_extractor catches a new canonical name in narration) and consequence upsert DM-side (consequence_extractor catches an implied stake in narration). S53 live verify ran a 27-turn stale exploration scene. Soft directive fired at stale=3 with text instructing the LLM to "lean toward compression unless tension warrants." The LLM, given a stale scene with no new pressure to render, invented a "Marla" innkeeper to fill the narrative space. The npc_extractor caught was_new=True. §1.F.c fired. Counter reset to 0. The very signal designed to detect "world progression" rewarded the LLM elaboration the directive was trying to suppress. Counter never reached stale=6 hard threshold across the entire scene.

**Context — instance 2 (S63, §1.F.e consequence upsert DM-side):**
S63 opened as a proactive patch to drop §1.F.e per identical-shape rule to S53's §1.F.c drop. Inventory-before-patch recon found §1.F.e was specced in v1 but **never wired** in the S56 Quest Layer implementation — `_reset_scene_stale` is called from 6 sites in `discord_dnd_bot.py` (combat-start, rest event, Avrae roll, /play, /compress, /travel) and from zero sites in the consequence-extraction path. The bot has been running on §1.F (a)/(b)/(d) only since v1 ship. The harm S53 surfaced for §1.F.c was prevented for §1.F.e by implementation gap. S63 shipped a doc-only patch making the prevention explicit + dropping §1.F.e from the locked list.

**Failure mode:** §1.F resets fire on signals interpreted as "world progressed." Operator-driven and Avrae-driven signals (location_change, advance_time, Avrae roll) cannot be forged — they require operator typing or mechanical dice. LLM-extracted signals (was_new, consequence_upsert, future T2 player-closure-intent) are derived from narration content. In a stagnation-detection context the LLM is the agent being measured; reading its narration to score "did the world progress" inverts the detection. The LLM elaborating into a stale scene is exactly the failure mode the directive is detecting; an extractor catching that elaboration cannot then score it as "world progression" without subverting the measurement.

**Architectural relationship:** Distinct from §76 (LLM-writable persisted state contamination). LLM-extracted signals here don't write persisted state — they trigger an in-memory counter reset. The §76 four-property test passes (counter is not persisted, not retrieved into prompt). The failure is at a different surface: **signal-source provenance in a measurement context.** Sister failure to §F-08 (Layer 2 narration drift — NPCs never commit). §F-08 is about LLMs not advancing scenes; this is the inverse — LLMs *appearing* to advance via narration elaboration that registers as world progression. Same load-bearing observation: LLM narration alone isn't a structural advancement signal.

**Disposition (post-S63 anchored at spec level):** Scene Lifecycle v1 §1.F restricted to operator-driven and Avrae-driven signals only. Per §12.10: "§1.F should restrict to operator-driven or Avrae-driven signals only (location change, advance_time, Avrae roll) — signals the LLM cannot generate by narration alone." Future motion-system primitives that consider an activity-signal set in a stagnation-detection (or analogous measurement) context should not include LLM-extracted signals without an additional corroboration requirement (e.g., co-occurrence with a non-LLM signal in the same turn — the corroborated-signal pattern filed in §12.10 as v1.x candidate).

**Why CANDIDATE not anchored:** Two instances cross-reference (NPC was_new wired+dropped vs consequence upsert specced+never-wired+formally-dropped). The architectural rule is empirically grounded but second-instance came via implementation gap rather than independent failure. Anchoring waits for either operator confirmation of the structural rule OR a third instance arising independently. The pattern is named here so future ships read it; doctrine-level anchoring is the operator's call.
