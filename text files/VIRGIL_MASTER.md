...
**Sx (post-S46 cleanup queue):** `npc_token_prefix_collapse` / `npc_anchor_ambiguous` log lines — emitted from `npc_upsert` in `dnd_engine.py` (DOCTRINE.md §14.1 doctrine exception). When the incoming canonical_name shares its leading whole-token with exactly ONE `skeleton_origin=1` row in the same campaign, route the upsert to that anchor's recency-bump UPDATE branch (mention_count + 1, last_mentioned = now); log `npc_token_prefix_collapse: campaign={N} incoming='{name}' anchor_id={id} anchor_name='{name}' (mention_count_bumped)`. When 2+ anchors match the leading whole-token, log `npc_anchor_ambiguous: campaign={N} incoming='{name}' anchors=[id={N} name='{name}', ...] (no_collapse)` and fall through to existing strict-equality default. Skeleton-authored upserts (`skeleton_origin=True`) skip the collapse path entirely — authored canon is not parser hits. Reuses existing `_is_token_prefix` helper (dnd_engine.py:3469). Four-constraint lock: unique anchor / skeleton_origin=1 / same campaign_id / whole-token. Tests: `test_npc_token_prefix_collapse.py` (41 assertions).
...
- **Scene extraction's narrative-detail surfaces deleted (S39).** Pre-Ship-2, `extract_scene_updates` ran a per-turn LLM call to summarize DM narration into `location` / `focus` / `established_details` / `open_questions` / `last_scene_change`. All five fields hit the Doctrine §76 four-property latent-canon test (LLM-writable + persisted + retrieved + narratively inferential) and were structurally deleted. The function now writes only `last_player_action` (verbatim player input — fails property 4 — kept as borderline 3+1 per spec §6.1 audit). The LLM has no per-turn scene-summary write path; canonical scene anchors flow via `set_current_location` (FK → `location_label` derived label), `last_dm_response` (single-writer-disciplined prior narration verbatim), and skeleton-loaded canon. Mechanical/pacing fields stay deterministic-only as ever.
...
- **NPC state-sync via §1b suggester (Ship 3, S41 post-pivot).** Pre-Ship-3, hydration's engine-side stats and Avrae's combatant store were intentionally divergent — combat against hydrated NPCs hit Finding H (`<None>` HP, mechanical-vs-narrated mismatch). Ship 3 closes that gap via the `_avrae_project_npc` helper in `discord_dnd_bot.py`: when a hydrated NPC needs Avrae sync, the helper posts a 3-line copy-paste block to `#dm-aside` (`!init remove <name>` + `!init add <init_mod> <name> -hp <hp>` + `!init opt <name> -ac <ac>`); the DM pastes each line as a separate message in `#dm-narration`; Avrae executes (Avrae responds to human-typed commands only — bot-emitted `!`-prefixed commands are silently filtered, a structural API boundary empirically discovered S41). Two trigger surfaces: `/hydrate` slash (Case A, active) and `_handle_init_list_event` hydration branch (Case B, passive). Case A mid-combat re-hydrate warns about HP-reset risk; Case B silently no-ops when combatant has numeric HP (Avrae's mid-combat state authoritative). The §1b doctrine governs the pattern: bot suggests, deterministic gates validate, DM approves by paste, Avrae executes. This is §1b's second project instance (first: Track 6 #5.1 SRD suggester). `/hydrate` itself is an emergency-fix surface — the canonical NPC entry flow is the DM typing `!init add <init> <name> -hp <hp> -ac <ac>` directly with full stats inline.
...
- **Locked violation classes (§11.F): five classified + one sentinel.** This is a stability promise about classification *semantics*, not about count — adding a new classified violation type requires spec amendment (it changes what the verifier considers a real violation). Sentinels can be added for operational distinctions (e.g. verifier crash vs verified-clean) but must never represent a classified violation — they sit outside the first-violation-wins ordering and never trigger retry escalation.
  - **Classified (5):** `FABRICATED_COMBATANT`, `VERDICT_CONTRADICTION`, `STATE_MUTATION_CLAIM`, `ACTOR_OMISSION`, `ROLL_OUTCOME_DRIFT`. First-violation-wins ordering: see `narration_verifier.py` for canonical sequence.
  - **Sentinel (1):** `VERIFIER_ERROR` — `narration_verifier` raised an exception; emitted from `dnd_engine.py:6485` fallback path. Distinguishes verifier-crashed from verified-clean. Outside the classification ordering by construction; fail-open invariant preserved (`passed=1` on fallback path).
...
**Single write paths:**
- `npc_upsert()` — only writer for `dnd_npcs`. Six branches: insert / parser×parser / parser→skeleton promotion / skeleton×skeleton reload / skeleton lock / unique-skeleton-anchor token-prefix collapse (DOCTRINE.md §14.1).
