...
# S72.2 — Conversational-Runtime Inversion v0 §76 hygiene closure.
#
# Path B closure on chroma DM-stores: retrieve only role='user' (player-turn
# narration) to break the recursive contamination loop on the read side.
# Write side (chroma_store('dm', response) at discord_dnd_bot.py:3526) is
# unchanged — asymmetric closure preserves write substrate for future
# architectural use; structurally breaks the §76 4/4 + 5/6 + 6/6 surface.
#
# Anchored against DOCTRINE.md §76.1 (rate-unlimited write — chroma_store
# fires every DM turn) + §76.2 (verbatim re-injection — `[{ts}] DM: {doc[:200]}`
# was truncated-but-unsummarized).
#
# Feature-disable switch: set _CHROMA_DM_FILTER_DISABLED = True at module level
# to temporarily revert to pre-S72.2 behavior (returns both DM + user-role
# docs). Defense-in-depth — recon at S72.2 confirmed no downstream surface
# depends on DM-role retrieval, but operator-runtime override preserved.
_CHROMA_DM_FILTER_DISABLED = False


def chroma_search(campaign_id, query, n=4):
    """Pull recent relevant campaign turns. Empty string if no hits or no DB.

    S72.2 Path B closure: filters to role='user' (player turns) only by
    default. DM-role docs (LLM-written narration) are excluded — structurally
    breaks the §76 recursive contamination loop on the read side. See
    DOCTRINE.md §76.1 + §76.2 for the loop-closure framing.

    Override: set `dnd_engine._CHROMA_DM_FILTER_DISABLED = True` at runtime
    to revert pre-S72.2 behavior (returns DM + user docs). Telemetry logs
    every search with role-filter status for audit.
    """
    ...
    try:
        ...
        if embedding is None:
            return ""
        # S72.2: native `where` filter pushes role='user' down to the collection
        # layer (efficient at scale; chroma_dnd is ~4GB at S72.2 ship time).
        # Feature-disable returns pre-S72.2 behavior.
        if _CHROMA_DM_FILTER_DISABLED:
        ...
        # S72.2 telemetry — always-fire per §11.8 lock pattern. Surfaces
        # empirical chroma retrieval shape for §76 closure validation.
        log(f"chroma_search: campaign={campaign_id} filter={filter_mode} "
            f"returned={len(lines)} distance_filtered={distance_filtered} "
            f"n_requested={n}")
...
def db_init():
    ...
    # S73 fresh-DB guard: PRAGMA table_info returns empty list for a
    # non-existent table, which previously caused the ALTER to crash on
    # fresh DBs (test environments). Skip ALTER if table doesn't yet exist;
    # the dnd_npcs CREATE TABLE below includes the column directly via the
    # canonical schema, so production rows always have pronouns.
    npc_cols = [r[1] for r in conn.execute("PRAGMA table_info(dnd_npcs)").fetchall()]
    if npc_cols and 'pronouns' not in npc_cols:
    ...
    # Conversational-Runtime Inversion v0 — Phase 3a (S73). NPC utterance
    # commitment tracking table. Distinct from S19's compute_commitment_directive
    # (player-action-honor escape directive). This table tracks NPC-side
    # commitments uttered in narration ("I'll bring supplies by morning")
    # for anti-gaslight rails at S75 (Phase 3c — parse_npc_commitment_utterances
    # extractor + compute_npc_commitment_anti_gaslight_directive). Schema ships
    # at S73 so the table exists when the extractor wires in at S75. No writes
    # at S73 (no extractor yet); writer stub `npc_commitment_upsert` defined for
    # idempotent contract.
    #
    # commitment_kind enum: 'price' / 'action' / 'promise' / 'refusal' / 'other'.
    # status enum: 'open' / 'resolved' / 'contradicted' / 'expired'.
    #
    # Foreign key to dnd_npcs.id with ON DELETE CASCADE — when an NPC is
    # deleted, their commitments cascade. PRAGMA foreign_keys=ON is set at
    # engine init per S61. Cascade-firing helpers (npc delete paths) inherit
    # the same connection-PRAGMA discipline.
    conn.execute('''CREATE TABLE IF NOT EXISTS dnd_npc_commitments (
        id               INTEGER PRIMARY KEY AUTOINCREMENT,
        campaign_id      INTEGER NOT NULL,
        npc_id           INTEGER NOT NULL REFERENCES dnd_npcs(id) ON DELETE CASCADE,
        commitment_text  TEXT    NOT NULL,
        commitment_kind  TEXT    NOT NULL DEFAULT 'other',
        uttered_turn     INTEGER,
        status           TEXT    NOT NULL DEFAULT 'open',
        created_at       TEXT    NOT NULL,
        updated_at       TEXT    NOT NULL
    )''')
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_npc_commitments_campaign_npc "
        "ON dnd_npc_commitments(campaign_id, npc_id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_npc_commitments_campaign_status "
        "ON dnd_npc_commitments(campaign_id, status)"
    )
...
def get_focused_quest(campaign_id: int) -> dict | None:
    """S73.2 — return the single in-progress quest currently in focus.

    Resolution order:
      1. `dnd_scene_state.current_act_id` set → resolve to act → parent quest;
         if that quest is still in-progress, return it. (Composition Layer v0
         maintains this anchor; `quest_accept` sets it on quests with acts.)
      2. Fallback: most-recently-accepted in-progress quest (ORDER BY
         accepted_turn DESC NULLS LAST, id DESC LIMIT 1). For quests authored
         via /quest add without acts, accepted_turn may be NULL — id DESC
         tie-break gives newest-row-first behavior.
      3. None when no in-progress quests exist.

    Used by the footer renderer (discord_dnd_bot.py) to show a single
    focused-quest reminder instead of a comma-list of all in-progress quests.
    `/quest list` remains the full-ledger view.

    Defensive: even if current_act_id points at an act whose quest is no
    longer in-progress (e.g., race condition between quest_status change and
    Composition Layer cleanup), this falls through to the fallback rule.
    Pure read; no writes.
    """
    # Step 1 — try current_act anchor
    try:
        act = get_current_act(campaign_id)
        if act and act.get('quest_id'):
            q = get_quest_by_id(campaign_id, int(act['quest_id']))
            if q and q.get('status') == 'in-progress':
                return q
    except Exception as e:
        log(f"get_focused_quest: current_act resolve error "
            f"campaign={campaign_id} err={e!r}")

    # Step 2 — fallback: most-recently-accepted in-progress
    conn = sqlite3.connect(DB_PATH)
    try:
        row = conn.execute(
            f"SELECT {_QUEST_SELECT_COLS} FROM dnd_quests "
            "WHERE campaign_id=? AND status='in-progress' "
            "ORDER BY accepted_turn DESC NULLS LAST, id DESC "
            "LIMIT 1",
            (campaign_id,)
        ).fetchone()
    finally:
...
# ── Conversational-Runtime Inversion v0 — dnd_npc_commitments accessors ─────
#
# Phase 3a (S73) ships the schema + §17 single-writer + read accessor. No
# parser-side wiring yet — `parse_npc_commitment_utterances` (extractor) and
# `compute_npc_commitment_anti_gaslight_directive` (prompt directive) land at
# Phase 3c (S75). Single-writer discipline preserved from S73 forward.
#
# NAMING NOTE (S72 namespace lock): this surface uses the `npc_commitment_*`
# prefix to disambiguate from S19's `compute_commitment_directive`
# (player-action-honor escape directive at dnd_orchestration.py:1954).

_VALID_NPC_COMMITMENT_KINDS = frozenset(
    {'price', 'action', 'promise', 'refusal', 'other'}
)
_VALID_NPC_COMMITMENT_STATUSES = frozenset(
    {'open', 'resolved', 'contradicted', 'expired'}
)


def npc_commitment_upsert(campaign_id: int, npc_id: int,
                          commitment_text: str,
                          commitment_kind: str = 'other',
                          uttered_turn: int = None,
                          status: str = 'open') -> int | None:
    """§17 single-writer for dnd_npc_commitments. INSERT semantics at v0
    (no in-place mutation — re-utterance writes a new row, the extractor
    handles dedup via cross-turn LRU per N-1 hint extractor precedent).

    Returns the new row's id, or None if validation refuses. Validation:
      - commitment_text non-empty after strip
      - commitment_kind in _VALID_NPC_COMMITMENT_KINDS
      - status in _VALID_NPC_COMMITMENT_STATUSES
      - npc_id exists in dnd_npcs (FK validation; otherwise FK constraint
        would refuse at SQLite level, but we surface a clean log line)

    No reads of this table land in LLM prompt at Phase 3a — anti-gaslight
    directive injection lands at Phase 3c (S75).
    """
    text = (commitment_text or '').strip()
    if not text:
        log(f"npc_commitment_upsert: refused empty text "
            f"campaign={campaign_id} npc_id={npc_id}")
        return None
    if commitment_kind not in _VALID_NPC_COMMITMENT_KINDS:
        log(f"npc_commitment_upsert: refused bad kind={commitment_kind!r} "
            f"campaign={campaign_id} npc_id={npc_id}")
        return None
    if status not in _VALID_NPC_COMMITMENT_STATUSES:
        log(f"npc_commitment_upsert: refused bad status={status!r} "
            f"campaign={campaign_id} npc_id={npc_id}")
        return None
    conn = sqlite3.connect(DB_PATH)
    try:
        ...
        if not npc_check:
            log(f"npc_commitment_upsert: refused npc_not_found "
                f"campaign={campaign_id} npc_id={npc_id}")
            return None
        now_iso = _now()
        cur = conn.execute(
            "INSERT INTO dnd_npc_commitments "
            "(campaign_id, npc_id, commitment_text, commitment_kind, "
            " uttered_turn, status, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (campaign_id, npc_id, text, commitment_kind,
             uttered_turn, status, now_iso, now_iso)
        )
        new_id = cur.lastrowid
        conn.commit()
        log(f"npc_commitment_upsert: campaign={campaign_id} npc_id={npc_id} "
            f"id={new_id} kind={commitment_kind} status={status}")
        return new_id
    finally:
        conn.close()


def npc_commitment_set_status(commitment_id: int, status: str) -> bool:
    """Transition a commitment row's status. Returns True if updated.

    Used at S75 by the anti-gaslight directive when the extractor detects
    a resolution / contradiction / expiry. Not exercised at Phase 3a; defined
    so the single-writer contract is complete from S73 forward.
    """
    if status not in _VALID_NPC_COMMITMENT_STATUSES:
        log(f"npc_commitment_set_status: refused bad status={status!r} "
            f"id={commitment_id}")
        return False
    conn = sqlite3.connect(DB_PATH)
    try:
        cur = conn.execute(
            "UPDATE dnd_npc_commitments SET status=?, updated_at=? "
            "WHERE id=?",
            (status, _now(), commitment_id)
        )
        updated = cur.rowcount > 0
        conn.commit()
        if updated:
            log(f"npc_commitment_set_status: id={commitment_id} status={status}")
        return updated
    finally:
        conn.close()


def get_active_npc_commitments(campaign_id: int,
                                npc_id: int = None) -> list:
    """Return open commitments for the campaign, optionally filtered to
    a specific npc_id. Ordered by uttered_turn DESC NULLS LAST, then id ASC.

    Used at S75 by the anti-gaslight directive to inject open commitments
    into prompt context. Not exercised at Phase 3a; defined for read-path
    ...
    try:
        if npc_id is not None:
            rows = conn.execute(
                "SELECT id, campaign_id, npc_id, commitment_text, "
                "commitment_kind, uttered_turn, status, created_at, updated_at "
                "FROM dnd_npc_commitments "
                "WHERE campaign_id=? AND npc_id=? AND status='open' "
                "ORDER BY uttered_turn DESC NULLS LAST, id ASC",
                (campaign_id, npc_id)
            ).fetchall()
        else:
            rows = conn.execute(
    ...
    return [
        {
            'id': r[0], 'campaign_id': r[1], 'npc_id': r[2],
            'commitment_text': r[3], 'commitment_kind': r[4],
            'uttered_turn': r[5], 'status': r[6],
            'created_at': r[7], 'updated_at': r[8],
        }
        for r in rows
    ]
